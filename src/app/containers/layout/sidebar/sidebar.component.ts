...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    this.permissionSubscription = this.authService.authPermission.subscribe(permissions => {
  ...
  filteredMenuItems(menuItems: IMenuItem[]): IMenuItem[] {
    return menuItems ?
      menuItems.filter(
        _mainmenu => _mainmenu.subs ?
          this.filteredMenuItems(_mainmenu.subs).length :
          _mainmenu.role == this.defaultMenu || this.authService.is_main_store_login || (this.permissions?.some(bid => bid.url === _mainmenu.role))) : []
  }
}
