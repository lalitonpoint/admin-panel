...
@Component({
  selector: 'app-open-ride-earning-details-modal',
  templateUrl: './open-ride-earning-details-modal.component.html',
  styleUrls: ['./open-ride-earning-details-modal.component.scss']
})
export class OpenRideEarningDetailsModalComponent {
  modalRef: BsModalRef;
  config = {
    backdrop: true,
  ...
  getTripDetails() {
    ...
    this.earningService.openRideStatementProviderEarning(json).then(res => {
      this.totalcharge = 0;
      this.tripdetail = res.detail[0];

      const tripStatus = res.detail[0].trip_status;
      tripStatus?.sort((a, b) => {
